...
from ot2_rest_node import OT2Node
...
class Opentrons_Resources:
    ...
    def parse_logfile(self, ot_log):
        ...
        OT2Node.logger.log("Log File" + log_path + " Grabbed")
        with open(log_path, 'r') as file:
            ot_log = json.load(file)
        
        OT2Node.logger.log(ot_log)
        
        #two passes, one for all "load labware" commands, and one for all actions

        OT2Node.logger.log("Processing setup commands")
        self.process_setup_commands(ot_log)
        OT2Node.logger.log("Processing protocol commands")
    ...
    def process_setup_commands(self, ot_log):
        ...
        OT2Node.logger.log("COMMANDS")
        OT2Node.logger.log(commands)

        for command in commands:
            cmd_type = command.get('commandType')
            OT2Node.logger.log("COMMAND")
            OT2Node.logger.log(command)
            OT2Node.logger.log("COMMAND TYPE")
            OT2Node.logger.log(cmd_type)
            
            if cmd_type == 'loadLabware':
                OT2Node.logger.log("LOADING LABWARE")
                self.load_labware(command, ot_log)
            elif cmd_type == 'loadPipette':
                OT2Node.logger.log("LAODING PIPETTE")
        ...
        for command in commands:
            ...
            try:
                if cmd_type == 'pickUpTip':
                    ...
                    OT2Node.logger.log("PICKING UP TIP")
                elif cmd_type == 'aspirate':
                    OT2Node.logger.log("ASPIRATING")
                    self.aspirate(command, ot_log)
                elif cmd_type == 'dispense':
                    OT2Node.logger.log("DISPENSING")
                    self.dispense(command, ot_log)
                elif cmd_type == 'dropTip':
                    OT2Node.logger.log("DROPPING TIP")
    ...
    def load_labware(self, command, ot_log):
        ...
        OT2Node.logger.log("LABWARE PARAMS AND RESULT")
        OT2Node.logger.log("PARAMS", params)
        OT2Node.logger.log("RESULT", result)
        ...
        OT2Node.logger.log("LOCATION, LOADNAME, AND SLOT NAME")
        OT2Node.logger.log("LOCATION", location)
        OT2Node.logger.log("LOADNAME", load_name)
        OT2Node.logger.log("SLOTNAME", slot_name)
        ...
        for labware in ot_log.get('data', {}).get('labware', []):
            if (labware.get('loadName') == load_name and
                ...
                OT2Node.logger.log("MATCHING ID FOUND")
                break
        
        if not labware_id:
            OT2Node.logger.log("MATCHING ID NOT FOUND")
        ...
        #store info
        OT2Node.logger.log("STORING LABWARE INFO")
        ...
        OT2Node.logger.log("LOOK FOR SLOT NAME IN DECK SLOTS")
        OT2Node.logger.log("SLOT NAME", slot_name)
        OT2Node.logger.log("DECK SLOTS", self.deck_slots)
        if slot_name in self.deck_slots:
            OT2Node.logger.log("FOUND SLOT RESOURCE", slot_resource)
            ...
            #    # Grid resource if tip rack
                OT2Node.logger.log("CHECKING IF RESOURCE IS A TIP RACK")
                OT2Node.logger.log("LOAD NAME LOWER", load_name.lower())
                if 'tip' in load_name.lower():
                    OT2Node.logger.log("IS TIP RACK")
            ...
            #        labware_resource = self.client.add_resource(labware_resource)
                    OT2Node.logger.log("CREATING TIP RACK RESOURCE FROM TEMPLATE")
                    OT2Node.logger.log("TEMPLATE NAME", load_name)
                    OT2Node.logger.log("RESOURCE NAME", labware_id)
                    ...
                    OT2Node.logger.log("LABWARE RESOURCE", labware_resource)
                    OT2Node.logger.log("SETTING LABWARE RESOURCE AS CHILD OF DECK SLOT")
                    OT2Node.logger.log("SLOT RESOURCE", slot_resource)
                    ...
                    OT2Node.logger.log("RESOURCE IS A PLATE")
                    OT2Node.logger.log("LOAD NAME LOWER", load_name.lower())
                   #plate as grid, either 96 or 384
                    ...
                    # )
                    # labware_resource = self.client.add_resource(labware_resource)
                    OT2Node.logger.log("CREATING PLATE RESOURCE FROM TEMPLATE")
                    OT2Node.logger.log("TEMPLATE NAME", load_name)
                    OT2Node.logger.log("RESOURCE NAME", labware_id)
                    labware_resource = self.client.create_resource_from_template(
                        template_name = load_name,
                    ...
                    OT2Node.logger.log("REGISTERING AS NEITHER PLATE OR TIP RACK")
    ...
    def load_pipette(self, command, ot_log):
        ...
        OT2Node.logger.log("PIPETTE PARAMS AND RESULT")
        OT2Node.logger.log("PARAMS", params)
        OT2Node.logger.log("RESULT", result)
