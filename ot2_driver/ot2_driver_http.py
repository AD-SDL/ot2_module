...
class OT2_Driver:
    ...
    def __init__(
        ...
        self.retry_strategy = Retry(total=retries, backoff_factor=retry_backoff, status_forcelist=retry_status_codes)
    ...
    def get_runs(self) -> Optional[List[Dict[str, str]]]:
        ...
        if runs_resp.status_code == 200:
            ...
            for run in runs_resp.json()["data"]:
                runs_simplified.append({"runID": run["id"], "protocolID": run["protocolId"], "status": run["status"]})

            return runs_simplified

        return None

    def get_robot_status(self) -> Status:
